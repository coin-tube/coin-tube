      ...
      <form onSubmit={handleSubmit(onSubmit)}>
          <div className= "regisInfo-box">
            <div className= "acinput-container">
              ...
              <input type="text" name="user_name" className="text-field" placeholder="name"
              {...register("user_name", { required: true, maxLength: 10 })}
              />{errors.user_name && errors.user_name.type === "required" && <p>This name field is required</p>}
                {errors.user_name && errors.user_name.type === "maxLength" && <p>Your input exceed maximum length</p>}

              <input type="password" name="password" className="text-field" placeholder="password"
              {...register("password", { required: true, minLength: 6 })}
              />{errors.password && errors.password.type === "required" && <p>This password field is required</p>}
              {errors.password && errors.password.type === "minLength" && <p>Password must have at least 6 characters</p>}
              <input type="password" name="password_check" className="text-field" placeholder="password_check"
                                  {...register("password_check", {
                                    required: true,
                                    validate: (value) =>
                                        value === password.current
                                })}
              />{errors.password_check && errors.password_check.type === "required" && <p>This password confirm field is required</p>}
              {errors.password_check && errors.password_check.type === "validate" && <p>The passwords do not match</p>}
              </div>
            <div className ="qr-container">
              <QRCode value={qrvalue} size={150} style={{ margin: "auto" }} />
              <div className = "qr-check-container">
                { AddressCheck == true ? "Success" : <Button variant="Success" className="qr_button" onClick={getUserAddress}><div className= "qr_load">QR_LOAD</div></Button> } 
              </div>
